...
var $author$project$Main$init = {counter: 0, data: '', statistics: $elm$core$Maybe$Nothing, timeSeries: $elm$core$Maybe$Nothing};
...
var $author$project$Compute$france20200225DataAsString = A2(
	$elm$core$String$join,
	', ',
	A2(
		$elm$core$List$map,
...
var $author$project$Compute$e = 2.7138;
...
var $jxxcarlson$elm_stat$Stat$maximum = F2(
	function (selector, dataList) {
		return $elm$core$List$maximum(
			A2($elm$core$List$map, selector, dataList));
	});
...
var $jxxcarlson$elm_stat$Stat$minimum = F2(
	function (selector, dataList) {
...
var $jxxcarlson$elm_stat$Data$xCoord = function (_v0) {
	var x = _v0.a;
	var y = _v0.b;
	return x;
};
var $jxxcarlson$elm_stat$Data$yCoord = function (_v0) {
	var x = _v0.a;
	var y = _v0.b;
	return y;
};
var $jxxcarlson$elm_stat$Stat$statistics = function (data) {
	var nn = $elm$core$List$length(data);
	var _v0 = nn < 2;
	if (_v0) {
		return $elm$core$Maybe$Nothing;
	} else {
		var ys = A2($elm$core$List$map, $jxxcarlson$elm_stat$Data$yCoord, data);
		var ySum = $elm$core$List$sum(ys);
		var xs = A2($elm$core$List$map, $jxxcarlson$elm_stat$Data$xCoord, data);
		var xsSquared = $elm$core$List$sum(
			A2(
				$elm$core$List$map,
				function (x) {
					return x * x;
				...
				xs));
		var xySum = $elm$core$List$sum(
			A3($elm$core$List$map2, $elm$core$Basics$mul, xs, ys));
		var xSum = $elm$core$List$sum(xs);
		var xMin = A2(
			$elm$core$Maybe$withDefault,
			0,
			A2($jxxcarlson$elm_stat$Stat$minimum, $jxxcarlson$elm_stat$Data$xCoord, data));
		var xMax = A2(
			$elm$core$Maybe$withDefault,
			0,
			A2($jxxcarlson$elm_stat$Stat$maximum, $jxxcarlson$elm_stat$Data$xCoord, data));
		var square = function (x) {
			return x * x;
		};
		var origin = _Utils_Tuple2(0, 0);
		var rightDataPoint = A2(
			$elm$core$Maybe$withDefault,
			origin,
			$elm$core$List$head(
				A2(
					...
					function (point) {
						return _Utils_eq(
							$jxxcarlson$elm_stat$Data$xCoord(point),
							xMax);
					},
					data)));
		var n = nn;
		var xMean = xSum / n;
		var xDeltaSquaredSum = $elm$core$List$sum(
			A2(
				$elm$core$List$map,
				function (x) {
					return square(x - xMean);
				},
				xs));
		var xStdev = $elm$core$Basics$sqrt(xDeltaSquaredSum / (n - 1));
		var yMean = ySum / n;
		var ssTot = $elm$core$List$sum(
			A2(
				$elm$core$List$map,
				function (y) {
					return square(y - yMean);
				},
				ys));
		var yDeltaSquaredSum = $elm$core$List$sum(
			A2(
				$elm$core$List$map,
				function (y) {
					return square(y - yMean);
				...
				ys));
		var yStdev = $elm$core$Basics$sqrt(yDeltaSquaredSum / (n - 1));
		var leftDataPoint = A2(
			$elm$core$Maybe$withDefault,
			origin,
			$elm$core$List$head(
				A2(
					...
					data)));
		var determinant = n * xDeltaSquaredSum;
		var m = (1 / determinant) * ((n * xySum) - (xSum * ySum));
		var b = (1 / determinant) * (((-xSum) * xySum) + (xsSquared * ySum));
		var fs = A2(
			$elm$core$List$map,
			function (x) {
				return (m * x) + b;
		...
		var leftRegressionPoint = _Utils_Tuple2(
			$jxxcarlson$elm_stat$Data$xCoord(leftDataPoint),
			(m * $jxxcarlson$elm_stat$Data$xCoord(leftDataPoint)) + b);
		var rightRegressionPoint = _Utils_Tuple2(
			$jxxcarlson$elm_stat$Data$xCoord(rightDataPoint),
			(m * $jxxcarlson$elm_stat$Data$xCoord(rightDataPoint)) + b);
		return $elm$core$Maybe$Just(
			{b: b, leftDataPoint: leftDataPoint, leftRegressionPoint: leftRegressionPoint, m: m, n: nn, r2: r2, rightDataPoint: rightDataPoint, rightRegressionPoint: rightRegressionPoint, xMax: xMax, xMean: xMean, xMin: xMin, xStdev: xStdev, yMean: yMean, yStdev: yStdev});
	}
};
var $author$project$Compute$statistics = function (timeSeries_) {
	return $jxxcarlson$elm_stat$Stat$statistics(
		A2(
...
var $author$project$Compute$logStatistics = function (timeSeries_) {
	return $author$project$Compute$statistics(
		A2(
			$elm$core$List$map,
			$elm$core$Basics$logBase($author$project$Compute$e),
			timeSeries_));
};
...
var $author$project$Compute$timeSeries = function (data) {
	return $elm_community$maybe_extra$Maybe$Extra$combine(
		A2(
			...
			A2(
				...
				A2($elm$core$String$split, ',', data))));
};
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'Compute':
				var timeSeries = $author$project$Compute$timeSeries(model.data);
				var stats = A2($elm$core$Maybe$andThen, $author$project$Compute$logStatistics, timeSeries);
				return _Utils_update(
					model,
					{statistics: stats, timeSeries: timeSeries});
			case 'GotText':
				var str = msg.a;
				return _Utils_update(
					model,
			...
			case 'Clear':
				return _Utils_update(
					model,
					{data: '', timeSeries: $elm$core$Maybe$Nothing});
			case 'SampleData':
				return _Utils_update(
					model,
					{
						data: $author$project$Compute$france20200225DataAsString,
						timeSeries: $author$project$Compute$timeSeries($author$project$Compute$france20200225DataAsString)
					});
			default:
				return model;
		}
...
var $author$project$Main$markdownText = function (model) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				A2($jxxcarlson$elm_markdown$Markdown$Render$toHtml, $jxxcarlson$elm_markdown$Markdown$Option$ExtendedMath, $author$project$Strings$text))
			]));
};
...
var $author$project$Main$leftColumn = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				...
				$author$project$Main$markdownText(model))
			]));
};
	...
	_List_fromArray(
		[
			...
			$author$project$Main$buttonStyle,
			{
				...
				onPress: $elm$core$Maybe$Just($author$project$Main$Clear)
			})
		]));
var $author$project$Main$Compute = {$: 'Compute'};
var $author$project$Main$computeButton = A2(
	$mdgriffith$elm_ui$Element$row,
	$author$project$Main$buttonStyle,
	_List_fromArray(
		[
			...
			$author$project$Main$buttonStyle,
			{
				label: A2(
					...
					_List_fromArray(
						[$mdgriffith$elm_ui$Element$centerX, $mdgriffith$elm_ui$Element$centerY]),
					$mdgriffith$elm_ui$Element$text('Doubling time')),
				onPress: $elm$core$Maybe$Just($author$project$Main$Compute)
			})
		]));
var $elm$core$Basics$pow = _Basics_pow;
var $author$project$Compute$exponentialError = F3(
	function (m, b, data) {
		var nRoot = $elm$core$Basics$sqrt(
			$elm$core$List$length(data));
		var f = function (t) {
			return A2($elm$core$Basics$pow, $author$project$Compute$e, (m * t) + b);
		};
		return function (x) {
			return x / nRoot;
		}(
...
var $author$project$Compute$doublingTime = function (timeSeries_) {
	var _v0 = $author$project$Compute$logStatistics(timeSeries_);
	if (_v0.$ === 'Nothing') {
		return $elm$core$Maybe$Nothing;
	} else {
		var stats = _v0.a;
		...
		var data = A2(
			...
			timeSeries_);
		var error = A3($author$project$Compute$exponentialError, stats.m, stats.b, data);
		return $elm$core$Maybe$Just(
			{
				doublingTime: A2($elm$core$Basics$logBase, $author$project$Compute$e, 2.0) / k,
				error: error,
				r2: stats.r2
			});
	}
};
var $author$project$Compute$roundTo = F2(
	function (k, x) {
...
var $author$project$Main$dataSummary = function (model) {
	var _v0 = model.timeSeries;
	if (_v0.$ === 'Nothing') {
		return A2(
			$mdgriffith$elm_ui$Element$el,
			_List_fromArray(
			...
			$mdgriffith$elm_ui$Element$text('No valid data yet'));
	} else {
		var data = _v0.a;
		var n = $elm$core$String$fromInt(
			$elm$core$List$length(data));
		var doublingTimeData = $author$project$Compute$doublingTime(data);
		var doubling = function () {
			if (doublingTimeData.$ === 'Nothing') {
				return 'Bad data, could not compute doubling time';
			} else {
				var dd = doublingTimeData.a;
				return 'doubling time = ' + $elm$core$String$fromFloat(
					A2($author$project$Compute$roundTo, 2, dd.doublingTime));
			}
		}();
		var dataPoints = n + ' data points';
		var message = A2(
			$elm$core$String$join,
			', ',
			_List_fromArray(
...
var $author$project$Main$SampleData = {$: 'SampleData'};
var $author$project$Main$getSampleButton = A2(
	$mdgriffith$elm_ui$Element$row,
	$author$project$Main$buttonStyle,
	_List_fromArray(
		[
			...
			$author$project$Main$buttonStyle,
			{
				label: A2(
					...
					_List_fromArray(
						[$mdgriffith$elm_ui$Element$centerX, $mdgriffith$elm_ui$Element$centerY]),
					$mdgriffith$elm_ui$Element$text('Sample Data')),
				onPress: $elm$core$Maybe$Just($author$project$Main$SampleData)
			})
		]));
...
var $author$project$Main$textInput = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				...
				$mdgriffith$elm_ui$Element$height(
				$mdgriffith$elm_ui$Element$px(557)),
				$mdgriffith$elm_ui$Element$Font$size(14)
			]),
		{
			label: A2(
				...
				_List_Nil,
				$mdgriffith$elm_ui$Element$text('Enter data separated by commas')),
			onChange: $author$project$Main$GotText,
			placeholder: $elm$core$Maybe$Nothing,
			spellcheck: false,
			text: model.data
		});
};
var $author$project$Main$rightColumn = function (model) {
	var g = 0.9;
	return A2(
		$mdgriffith$elm_ui$Element$column,
		_List_fromArray(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$author$project$Main$textInput(model),
						$author$project$Main$dataSummary(model),
						A2(
						$mdgriffith$elm_ui$Element$row,
						_List_fromArray(
							[
						...
						_List_fromArray(
							[$author$project$Main$computeButton, $author$project$Main$clearButton, $author$project$Main$getSampleButton]))
					]))
			]));
};
var $author$project$Main$mainColumn = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$author$project$Main$leftColumn(model),
				$author$project$Main$rightColumn(model)
			]));
};
